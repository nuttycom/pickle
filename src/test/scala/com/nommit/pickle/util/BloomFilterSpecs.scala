...
object BloomFilterSpec extends Specification with ScalaCheck {
  import Prop._
  import scala.math._
  ...
  "append" should {
    "throw an exception on mismatched filter size" in {
      ...
      (filter1.append(filter2, 'f, 'g)) must throwAn[IllegalArgumentException]
    }

    "return a BloomFilter which contains all of each" in {
      val prop = forAll { (xs1: List[String], xs2: List[String], xs3: List[String]) =>
        val width = (max(xs1.length, xs2.length) + 1) * 2
        val filter1 = BloomFilter(xs1)(n=width)
        val filter2 = BloomFilter(xs2)(n=width)
        val filter = filter1.append(filter2, xs3: _*)

        xs1 mustNotExist { x => !filter.contains(x) }
        xs2 mustNotExist { x => !filter.contains(x) }
        xs3 mustNotExist { x => !filter.contains(x) }
      }
